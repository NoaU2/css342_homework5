...
#include "problem_1/list.h"

extern std::string case_id(int id);

extern std::string case_id(int id1, int id2);
...
void test_copy_constructor() {
    ...
    for (const auto &test_case: test_cases) {
        ...
        T this_list_1(other_list);
        ASSERT_TRUE(this_list_1 == other_list);

        T *plist = new T(this_list_1);
        ASSERT_TRUE(*plist == this_list_1);
        delete plist;

        T this_list_2(test_case);
        ASSERT_TRUE(this_list_2 == other_list);

        // verify other_list hasn't been changed
        for (int i = 0; i < test_case.size(); i++) {
            ASSERT_TRUE(test_case[i] == other_list[i]);
        }

        // check against shallow copy
        if (other_list.size() != 0) {
            other_list[0] = 99;
            ASSERT_TRUE(99 != this_list_1[0]);
            ASSERT_TRUE(99 != this_list_2[0]);
        }
    }
}
...
void test_reverse() {
    for (int n = 0; n < 10; n++) {
        T list_1(n);
        fill_list_from_back_with_sequence(n, list_1);
        T list_2(list_1);
        T expect(n);
        for (int i = n - 1; i >= 0; i--) {
            expect.push_back(i);
        }
        list_1.reverse_iterative();
        list_2.reverse_recursive();
        ASSERT_TRUE(expect == list_1) << case_id(n);
        ASSERT_TRUE(expect == list_2) << case_id(n);
    }
}
